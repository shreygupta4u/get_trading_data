...
scheduler.py
============
Runs fetch_nasdaq_candles.py daily at the configured time (default 4:15 PM).
...
-----
    ...
    python scheduler.py --run-now         # run today's fetch immediately + schedule
...
----------------------------
    ...
    Or register as a systemd service — see README / docs in the project folder.

Catch-up Logic
--------------
On each startup the scheduler checks whether today's data file already exists.
If it doesn't, and the current time is past SCHEDULE_HOUR:SCHEDULE_MINUTE,
it fetches immediately.  If the machine was off for multiple days, it also
back-fills up to CATCHUP_DAYS calendar days of missing trading-day data.
"""
...
# ── Logging ───────────────────────────────────────────────────────────────────
# Re-use the same rotating log file as the fetcher, just different logger name.
...
def missing_trading_days(lookback: int = config.CATCHUP_DAYS) -> list[date]:
    """
    Return a sorted list of trading days in the past `lookback` calendar days
    whose Parquet files are absent.  Excludes today (handled separately).
    """
    today = date.today()
    missing = []
    for offset in range(1, lookback + 1):          # start at yesterday
        d = today - timedelta(days=offset)
        if is_trading_day(d) and not parquet_path(d).exists():
            missing.append(d)
    missing.sort()
    return missing
...
def run_catchup() -> None:
    ...
    log.info(
        "Catch-up: fetching %d missed trading day(s): %s",
        len(missed),
        ", ".join(str(d) for d in missed),
    )
    for d in missed:
        log.info("  → Back-filling %s …", d)
        run_fetch(d)


def maybe_run_today() -> None:
    """
    If today's file is missing and we are past the scheduled time, fetch now.
    This covers the case where the machine was off during the cron window.
    """
    today = date.today()
    now = datetime.now()
    ...
    if now >= scheduled_time and not parquet_path(today).exists():
        log.info(
...
def main() -> None:
    ...
    # 1. Always run catch-up on startup
    run_catchup()
    maybe_run_today()
    ...
    if args.run_now:
        log.info("--run-now requested — fetching today …")
        run_fetch(date.today(), force=False)
